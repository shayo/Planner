function apt3fPointsInFunctionalVolume=fnPlotAverageBOLDResponseAuxBert(strctBOLD4D,iAnatomicalVolumeIndex,strROI,iSelectedFunctional, fWindowWidth, afRange)
...
afBoldResponseOptical = (afMeanBoldResponse(aiInterval + 20) + afMeanBoldResponse(aiInterval + 180))/2; 
afBoldResponseElectrical = (afMeanBoldResponse(aiInterval + 60) + afMeanBoldResponse(aiInterval + 140))/2;
afBoldResponseCombined = (afMeanBoldResponse(aiInterval + 100) + afMeanBoldResponse(aiInterval + 220))/2;
...
figure(21);
clf;hold on;
rectangle('position',[0 fMin 40 fMax-fMin],'facecolor',[0.6 0.6 0.6],'edgecolor','none');
