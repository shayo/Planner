...
int createRecordingChamber(vector<double> x_mm_list,
      ...
      try {
		...
		/*
		CComBSTR wrap_name(L"Wrap4");
		CComBSTR sketch_name(L"label_text");
	CComBSTR sketch_type(L"SKETCH");

		CComPtr<IDispatch> swDisp;
        CComPtr<IFeature> swFeature;
		CComPtr<ISketch> swSketch;
		CComPtr<ISketchText> swText;
		CComPtr<IWrapSketchFeatureData> swFeatData;
		
		
CComPtr<IPartDoc> swPart;
HRESULT hres = swModel->QueryInterface(__uuidof(IPartDoc), reinterpret_cast<void**>(&swPart));

	swPart->FeatureByName(wrap_name, &swDisp);
		swSketch= swDisp;
		//swFeature->GetDefinition(&swDisp);
		//swFeatData = swDisp;
 
		VARIANT *p;
		swSketch->GetSketchTextSegments(p);

		VARIANT_BOOL returnBoolean;
		swModel->ClearSelection();
	swSketch.GetSketchTextSegments()(0)
   		
	        CComBSTR plane = "Wrap4";
    VARIANT_BOOL  returnBoolean;
