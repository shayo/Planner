...
#define EPSF 2.2204460492503131e-016f
#ifndef max
...
static __inline double roundd(double r) {
    return (r > 0.0) ? floor(r + 0.5) : ceil(r - 0.5);
}

static __inline float roundff(float r) {
    return (r > 0.0f) ? floorf(r + 0.5f) : ceilf(r - 0.5f);
}

static __inline double absd(double r) { return (r > 0.0) ? r : -r; }
static __inline float absf(float r) { return (r > 0.0) ? r : -r; }
