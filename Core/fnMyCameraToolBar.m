function ret=fnMyCameraToolBar(varargin)
%fnMyCameraToolBar  Interactively manipulate camera.
%   fnMyCameraToolBar creates a new toolbar that enables interactive
...
%
%   fnMyCameraToolBar('NoReset') creates the toolbar without setting
%   any camera properties.
%
%   fnMyCameraToolBar('SetMode' mode) sets the mode of the
%   toolbar. Mode can be: 'orbit', 'orbitscenelight', 'pan',
%   'dollyhv', 'dollyfb', 'zoom', 'roll', 'nomode'.
%
%   fnMyCameraToolBar('SetCoordSys' coordsys) sets the principal axis
%   of the camera motion. coordsys can be: 'x', 'y', 'z', 'none'.
%
%   fnMyCameraToolBar('Show') shows the toolbar.
%   fnMyCameraToolBar('Hide') hides the toolbar.
%   fnMyCameraToolBar('Toggle') toggles the visibility of the toolbar.
%
%   fnMyCameraToolBar('ResetCameraAndSceneLight') resets the current
%   camera and scenelight.
%   fnMyCameraToolBar('ResetCamera') resets the current camera.
%   fnMyCameraToolBar('ResetSceneLight') resets the current scenelight.
%   fnMyCameraToolBar('ResetTarget') resets the current camera target.
%
%   MODE = fnMyCameraToolBar('GetMode') returns the current mode.
%   PAXIS = fnMyCameraToolBar('GetCoordSys') returns the current
%   principal axis.
%   VIS = fnMyCameraToolBar('GetVisible') returns the visibility.
%   H = fnMyCameraToolBar returns the handle to the toolbar.
%
%   fnMyCameraToolBar('Close') removes the toolbar.
%
%   fnMyCameraToolBar(FIG,...) specify figure handle as first argument.
%
...
%   $Revision: 1.26.4.22 $  $Date: 2008/11/04 21:19:41 $
...
if nin==0
    ...
    r = fnMyCameraToolBar(hfig,'show');
    fnMyCameraToolBar(hfig,'setmode', 'orbit')
    arg = '';
else
    arg = lower(vargin{1});
    if ~strcmp(arg, 'init') && ~strcmp(arg, 'motion') && ...
            ...
            isempty(Udata)
        r = fnMyCameraToolBar(hfig,'init');
        Udata = getUdata(hfig);
        %if ~strcmp(arg, 'nomode')
        %  scribeclearmode(hfig,'fnMyCameraToolBar', 'nomode');
        %end
    end
end
...
switch arg
    case 'down'
        ...
        if isfield(appdata,'doContext') && appdata.doContext
            if isfield(appdata,'CurrentObj') && ishandle(Udata.CurrentObj.Handle)
        ...
        set(hfig, 'windowbuttonupfcn', 'fnMyCameraToolBar(''up'')');
        pt = hgconvertunits(hfig,[0 0 get(hfig,'CurrentPoint')],...
            get(hfig,'Units'),'pixels',0);
        pt = pt(3:4);
        appdata.figStartPoint = pt;
        appdata.figLastPoint  = pt;
        appdata.figLastLastPoint = pt;
        appdata.buttondown = 1;
        appdata.moving = 0;
        setUdata(hfig,appdata)      
        ...
        %can call with fnMyCameraToolBar('down',0/1) to prohibit setting of windowbuttonfcn's
        switch get(hfig,'SelectionType')
            ...
            otherwise
                if strcmp(get(haxes,'warptofill'),'on') %& ...
                    %~(isappdata(haxes,'fnMyCameraToolBarAxesOptimized') & ...
                    %    isequal(getappdata(haxes,'fnMyCameraToolBarAxesOptimized'),1))
                    if iscameraobj(haxes)
                        axis(haxes,'vis3d');
                    else
                        return
                    end
                    %setappdata(haxes,'fnMyCameraToolBarAxesOptimized',1);
                ...
                if length(vargin)==1 || vargin{2}
                    set(hfig, 'windowbuttonmotionfcn', 'fnMyCameraToolBar(''motion'')')
                end
        end

        if haxes ~= Udata.hMyAxes 
            feval(Udata.wcb{1}{1},[],[],Udata.wcb{1}{2})
        end
    ...
    case 'up'
        ...
        if isfield(appdata,'CurrentObj') && haxes == appdata.hMyAxes
            set(appdata.CurrentObj.Handle,'ButtonDownFcn',appdata.CurrentObj.ButtonDownFcn);
    ...
    case 'keymotion'
        fnMyCameraToolBar(hfig,'down',false);
        ...
        if isstruct(Udata) && isfield(Udata,'figLastPoint')
            ...
            fnMyCameraToolBar(hfig,'motion');
            fnMyCameraToolBar(hfig,'up');
        end
    case 'stopmoving'
        Udata.moving = 0;
        setUdata(hfig,Udata)
    ...
    case 'keypress'
        ...
        switch get(hfig,'currentcharacter')
            case 'o'
                fnMyCameraToolBar(hfig,'setmode','orbit');
            case 'l'
                fnMyCameraToolBar(hfig,'setmode','orbitscenelight');
            case 'p'
                fnMyCameraToolBar(hfig,'setmode','pan');
            case 'd'
                fnMyCameraToolBar(hfig,'setmode','dollyhv');
            case 'z'
                fnMyCameraToolBar(hfig,'setmode','zoom');
            case 'r'
                fnMyCameraToolBar(hfig,'setmode','roll');
            case 'w'
                fnMyCameraToolBar(hfig,'setmode','walk');
            case 'D'
                fnMyCameraToolBar(hfig,'setmode','dollyfb');
            case char(28) %left
                fnMyCameraToolBar(hfig,'keymotion',[-1  0]);
            case char(29) %right
                fnMyCameraToolBar(hfig,'keymotion',[ 1  0]);
            case char(30) %up
                fnMyCameraToolBar(hfig,'keymotion',[ 0  1]);
            case char(31) %down
                fnMyCameraToolBar(hfig,'keymotion',[ 0 -1]);
            case 'c'
                postContextMenu(hfig,haxes);
    ...
    case 'resetall'
        h = [Udata.scenelights.h]; delete(h(ishghandle(h)));
        initUdata(hfig);
        updateToolbar(hfig)
        fnMyCameraToolBar('resetcameraandscenelight');
    ...
    case 'nomode'
        ...
        if isfield(Udata,'doContext') && Udata.doContext
            if isfield(Udata,'CurrentObj') && ishandle(Udata.CurrentObj.Handle)
    ...
    case 'init'
        ...
        ctb = findall(hfig, 'tag', 'fnMyCameraToolBar');
        if isempty(ctb)
    ...
    case 'close'
        ...
        fnMyCameraToolBar('stopmoving')
        h = [Udata.scenelights.h]; delete(h(ishghandle(h)));
        if any(ishghandle(Udata.mainToolbarHandle))
    ...
    otherwise
        error('MATLAB:fnMyCameraToolBar:unrecognizedinput','Unknown action string.');
end
...
while sum(abs(xy))> 0 && isstruct(Udata) && (flag || Udata.moving==1) && ishghandle(haxes)
    flag = 0;
    Udata = getUdata(hfig);
    % Force the axis to be vis3d
    if ishghandle(haxes)
        axis(haxes,'vis3d');
    end
    if ishghandle(haxes)
...
if length(dbstack)<get(0,'recursionlimit')-16
    ...
    while sum(abs(xy))> 0 && isstruct(Udata) && recursionflag && Udata.moving==1 && ishghandle(haxes)
    ...
    %
    %This is essentially the code in "fnMyCameraToolBar('up')"
    %it is copied here instead of referenced because calling
    %fnMyCameraToolBar('up') from here seems to cause timing issues.
    %Also, fnMyCameraToolBar('up') contains some checking for throws
...
while isstruct(Udata) && (flag || Udata.moving==1) && ishghandle(haxes)
    flag = 0;
    Udata = getUdata(hfig);
...
u = [];
props.ClickedCallback = 'fnMyCameraToolBar(''setcoordsys'', ''x'')';
...
props.ClickedCallback = 'fnMyCameraToolBar(''setcoordsys'', ''y'')';
props.ToolTip = 'Principal Axis Y';
props.CData = camtoolbarimages.y;
props.Tag = 'y';
u(end+1) = uitoggletool(props);

props.ClickedCallback = 'fnMyCameraToolBar(''setcoordsys'', ''z'')';
props.ToolTip = 'Principal Axis Z';
props.CData = camtoolbarimages.z;
props.Tag = 'z';
u(end+1) = uitoggletool(props);

props.ClickedCallback = 'fnMyCameraToolBar(''setcoordsys'', ''none'')';
...
u = [];
props.ClickedCallback = 'fnMyCameraToolBar(''togglescenelight'')';
props.ToolTip = 'Toggle Scene Light';
props.CData = camtoolbarimages.light;
u(end+1) = uipushtool(props,...
    'Separator', 'on');

props.ClickedCallback = 'fnMyCameraToolBar(''setprojection'', ''orthographic'')';
props.ToolTip = 'Orthographic Projection';
props.CData = camtoolbarimages.ortho;
u(end+1) = uipushtool(props,...
    'Separator', 'on');

props.ClickedCallback = 'fnMyCameraToolBar(''setprojection'', ''perspective'')';
props.ToolTip = 'Perspective Projection';
props.CData = camtoolbarimages.perspective;
u(end+1) = uipushtool(props);


props.ClickedCallback = 'fnMyCameraToolBar(''resetcameraandscenelight'')';
...
u = [];
props.ClickedCallback = 'fnMyCameraToolBar(''stopmoving'')';
...
set(Udata.mainToolbarHandle, 'tag', 'fnMyCameraToolBar', 'visible', 'off','serializable','off');
setUdata(hfig,Udata)
...
if isfield(Udata,'mainToolbarHandle') && ~any(ishghandle(Udata.mainToolbarHandle))
    Udata = [];
end
...
Udata = getUdata(hfig);
sl = Udata.scenelights;
index = ~ishghandle([sl.ax]); sl(index) = [];
index = ~ishghandle([sl.h]); sl(index) = [];
ax = haxes;
if isempty(sl)
    index = [];
else
...
if ~any(ishghandle(sl(index).h))
    h = light('parent',haxes);
    sl(index).h = h;
    set(h, 'visible', 'off', 'HandleVisibility', 'off', ...
        'tag', 'fnMyCameraToolBarScenelight')
end
...
persistent fnMyCameraToolBarInfoDialogShown

if isempty(fnMyCameraToolBarInfoDialogShown)
    fnMyCameraToolBarInfoDialogShown=0;
end

if ~fnMyCameraToolBarInfoDialogShown
    ax=haxes;
    fnMyCameraToolBarInfoDialogShown=1;
    [selectedButton,dlgShown]=uigetpref('fnMyCameraToolBar','donotshowinfodlg',...
...
set(hfig, 'windowbuttondownfcn',   'fnMyCameraToolBar(''down'')')
set(hfig, 'windowbuttonupfcn',     '')
%set(hfig, 'windowbuttonmotionfcn', '')
set(hfig, 'keypressfcn',           'fnMyCameraToolBar(''keypress'')');
...
menuTag='fnMyCameraToolBarContextMenu';
...
if isempty(h)
    ...
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'setmodegui','orbit'},hfig};
    oC = uimenu(props);
    ...
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'setmodegui','orbitscenelight'},hfig};
    oSL = uimenu(props);
    ...
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'setmodegui','pan'},hfig};
    oP = uimenu(props);
    ...
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'setmodegui','dollyhv'},hfig};
    oDH = uimenu(props);
    ...
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'setmodegui','dollyfb'},hfig};
    oDF = uimenu(props);
    ...
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'setmodegui','zoom'},hfig};
    oZ = uimenu(props);
    ...
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'setmodegui','roll'},hfig};
    oR = uimenu(props);
    ...
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'setcoordsys','x'},hfig};
    aX = uimenu(props);
    ...
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'setcoordsys','y'},hfig};
    aY = uimenu(props);
    ...
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'setcoordsys','z'},hfig};
    aZ = uimenu(props);
    ...
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'setcoordsys','none'},hfig};
    aN = uimenu(props);
    ...
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'resetcameraandscenelight'},hfig};
    rCS = uimenu(props);
    ...
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'resettarget'},hfig};
    rT = uimenu(props);
    ...
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'resetscenelight'},hfig};
    rS = uimenu(props);
    ...
    props.Separator = 'off';
    props.Tag = 'CameraReset_targetpoint';
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'setprojection','orthographic'},hfig};
    pO = uimenu(props);
    ...
    props.Separator = 'off';
    props.Tag = 'CameraReset_targetpoint';
    props.Callback = {@localUICallback,@fnMyCameraToolBar,{'setprojection','perspective'},hfig};
    pP = uimenu(props);
...
hCameraMotionTarget=findobj(hCameraMotionChildren,'tag',['CameraMode_' fnMyCameraToolBar(hfig,'getmode')]);
...
offon={'off','on'};
isActive=ismember(fnMyCameraToolBar(hfig,'getmode'), {'orbit' 'pan' 'walk'});
set(paxItems,'checked','off','enable',offon{isActive+1});

if isActive
    currPAx=fnMyCameraToolBar(hfig,'getcoordsys');
...
if strcmp(get(haxes,'warptofill'),'off')
    check='off';
    cbk={@localUICallback,@fnMyCameraToolBar,{'setaspectratio','normal'},hfig};
else
    check='on';
    cbk={@localUICallback,@fnMyCameraToolBar,{'setaspectratio','vis3d'},hfig};
end
...
if ~isempty(haxes)
    if ~isa(handle(haxes),'graph2d.legend') && ~isa(handle(haxes),'scribe.colorbar')
        val = true;
    else
        val = false;
    end
