function [x,fval,exitflag,output] = fnMyfminsearch(funfcn,x,options,varargin)
...
tic;
LastDraw=toc;
afYValue = Inf*ones(1,maxiter);
while func_evals < maxfun && itercount < maxiter
    afYValue(itercount) = fv(1);
    CurrTime=toc;
    if CurrTime-LastDraw > 0.5
        plot(options.Axes, 1:itercount, afYValue(1:itercount));
        drawnow
        LastDraw=CurrTime;
    end
    if g_bStop 
