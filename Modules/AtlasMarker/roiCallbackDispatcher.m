...
%   $Revision: 1.1.6.2 $  $Date: 2008/01/15 18:53:34 $
...
api.dispatchCallbacks         = @dispatchCallbacks;
    ...
    %----------------------------------------
	function dispatchCallbacks(callback_type) %#ok varargin needed by HG caller
	...
	function callback_list = getCallbackList(callback_type)
		...
		switch(lower(callback_type))
			...
			otherwise
				msg = 'Invalid callback_type. Valid strings are ''resizeDrag'',''translateDrag'', and ''newPosition''';
				error(sprintf('Images:%s:invalidCallbackType',mfilename),msg);
		end
